...
def resolve_municipality_code(municipality, base_url, api_key):
    ...
    if not municipalities:
        raise ForecastError("Meteocat no ha retornat dades de municipis.")
    ...
    raise ForecastError(f"No s'ha trobat el codi del municipi per a '{municipality}'.")
...
def build_payload():
    ...
    if not api_key:
        raise ForecastError("Falta la clau d'API de Meteocat (api_key).")
...
class ForecastHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path != "/forecast":
            self._send_json(404, {"error": "No trobat"})
            return
        try:
            ...
            self._send_json(500, {"error": f"Error inesperat: {exc}"})
            return
...
def main():
    ...
    print(f"Weather Forecast add-on escoltant al port {PORT}")
